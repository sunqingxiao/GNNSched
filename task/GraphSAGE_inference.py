...
from task.GraphSAGE import *

def import_func(model, graph):
    def test(model, graph):
        print(threading.currentThread().getName(),
            'GraphSAGE inference {} >>>>>>>>>>'.format(graph.name), time.time())
        model = model.cuda()
...
def import_model(data, graph):
    ...
    model = GraphSAGE(feat_dim, hidden_dim, num_classes, num_layers, 0.5)
    model.load_state_dict(torch.load('../data/model/GraphSAGE_{}_weights.pth'.format(name)))
    # set full name to disguish
    FULL_NAME = 'GraphSAGE_{}'.format(data)
