...
# GNNSched
def allocate_shared_cache() -> None:
    if is_initialized():
        torch._C._cuda_allocateSharedCache()

# GNNSched
def send_shared_cache() -> None:
    if is_initialized():
        torch._C._cuda_sendSharedCache()

# GNNSched
def recv_shared_cache() -> None:
    if is_initialized():
        torch._C._cuda_recvSharedCache()

# GNNSched
def insert_shared_cache_for_computation(size, offset) -> None:
    if is_initialized():
        torch._C._cuda_insertSharedCacheForComputation(size, offset)
