...
#include <iostream> // GNNSched
#include <unistd.h>  // GNNSched
#include <stdio.h>  // GNNSched
#include <sys/socket.h>  // GNNSched
#include <stdlib.h>  // GNNSched
#include <netinet/in.h>  // GNNSched
#include <string.h> // GNNSched
#include <arpa/inet.h>  // GNNSched
#define PORT 9010 // GNNSched
#define SIZE_SHARED_CACHE (20 * 1024UL * 1024UL * 1024UL) // GNNSched
...
class DeviceCachingAllocator {
 ...
 private:
  ...
  // GNNSched: Pre-allocated shared GPU memory
  void* COGNN_shared_ptr = NULL;
 ...
 public:
  ...
  /* GNNSched: allocate shared GPU memory */
  void allocateSharedCache() {
    std::lock_guard<std::recursive_mutex> lock(mutex);
    cudaError_t err = cudaMalloc(&COGNN_shared_ptr, SIZE_SHARED_CACHE);
    if (err != cudaSuccess) {
  ...
  /* GNNSched: send shared GPU memory */
  void sendSharedCache() {
    std::lock_guard<std::recursive_mutex> lock(mutex);
    cudaIpcMemHandle_t shared_cache_handle;

    // Pack CUDA pointer
    cudaError_t err = cudaIpcGetMemHandle(&shared_cache_handle, COGNN_shared_ptr);
    if (err != cudaSuccess) {
  ...
  /* GNNSched: recv shared GPU memory */
  void recvSharedCache() {
    ...
    // Extract the pointer
    cudaError_t err = cudaIpcOpenMemHandle(&COGNN_shared_ptr, shared_cache_handle, cudaIpcMemLazyEnablePeerAccess);
    if (err != cudaSuccess) {
  ...
  /* GNNSched: insert shared GPU memory to large block pool */
  void insertSharedCache(size_t size, size_t offset) {
    ...
    Block* block = new Block(
      ...
      size, 
      &large_blocks, 
      static_cast<char*>(COGNN_shared_ptr) + offset);
    // allocated_size += size;
    ...
    stat_types[static_cast<size_t>(StatType::AGGREGATE)] = true;
    stat_types[static_cast<size_t>(get_stat_type_for_pool(*(block->pool)))] = true;
    update_stat_array(stats.reserved_bytes, size, stat_types);
    return;
  }

  /* GNNSched: clear shared GPU memory */
...
class THCCachingAllocator {
 ...
 public:
  ...
  /* GNNSched: allocate shared GPU memory (devices) */
  void allocateSharedCache() {
    int count = device_allocator.size();
    for (int i = 0; i < count; i++)
      device_allocator[i]->allocateSharedCache();
  }

  /* GNNSched: send shared GPU memory (devices) */
  void sendSharedCache() {
    int count = device_allocator.size();
    for (int i = 0; i < count; i++)
      device_allocator[i]->sendSharedCache();
  }

  /* GNNSched: recv shared GPU memory (devices) */
  void recvSharedCache() {
    int count = device_allocator.size();
    for (int i = 0; i < count; i++)
      device_allocator[i]->recvSharedCache();
  }

  /* GNNSched: insert shared GPU memory to large block pool (devices) */
  void insertSharedCache(size_t size, size_t offset) {
    int count = device_allocator.size();
    for (int i = 0; i < count; i++)
      device_allocator[i]->insertSharedCache(size, offset);
  }
  ...
  void clearSharedCache() {
    int count = device_allocator.size();
    for (int i = 0; i < count; i++)
      device_allocator[i]->clearSharedCache();
  }
